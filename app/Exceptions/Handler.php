...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Throwable $exception)
    {
        ...
        if($exception instanceof ValidationException) {
            foreach($exception->errors() as $key => $index)
            {
                array_push($appendString,implode(",",$index));
            }
            $exception = new ParameterException("Parameter/Nilai inputan anda salah! " . $appendString[0] ?? null);
        }
        
        if ($exception instanceof ParameterException && !empty($appendString)) {
            $resultOutput->responseValidation = $appendString;
        }
